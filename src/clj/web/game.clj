...
(defn game-diff-json
  ...
  [uid gameid side {:keys [runner-diff corp-diff spect-diff]}]
  (let [diff (cond
               (= side "Corp")
               corp-diff
               (= side "Runner")
...
(defn send-state-diffs
  ...
  (doseq [{:keys [uid side]} (concat players spectators)
          :when (some? uid)]
    (ws/chsk-send! uid [:game/diff (game-diff-json uid gameid side diffs)])))
...
(defn select-state [uid lobby side {:keys [runner-state corp-state spect-state]}]
